...
// (e.g. DownloadJob.WaitForCompletion / WaitForNews use gated handlers and
// assert prompt signal delivery within a window). Under xUnit's default
// parallel execution on CPU-constrained CI runners (2 cores), a blocking test
// can prevent a timing-sensitive test's threads from being scheduled within
// its window, causing flaky failures. Serial execution removes that contention.
...
/// test that spawns threads/tasks or relies on prompt scheduling gets them
